    ...
    def solve_one(self, domains=None, to_do=None):
        ...
        if any(len(new_domains[var]) == 0 for var in domains):
            ...
            if var:
                ...
                self.display(
                    3, " adding", to_do if to_do else "nothing", "to to_do.")
    ...
    def heuristic(self, n):
        ...
        for v in self.variables[len(n):]:
            for d in self.csp.domains[v]:
                if d not in used:  # if not used before
                    c += self.csp.cost[(v, d)]  # calcualting the cost
                    i += 1  # inc
        return c/i if i > 0 else 0  # return 0 if i is 0 i.e No solution or return c/i
...
Con_solver.max_display_level = 0  # display details of AC (0 turns off)
# display details of AC (0 turns off)
...
# Searcher.max_display_level = 4  # display search trace (0 turns off)
