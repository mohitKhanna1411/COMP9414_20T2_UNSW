...
class Searcher(Displayable):
    ...
    def search(self):
        ...
        while not self.empty_frontier():
            ...
            self.num_expanded += 1
            if self.problem.is_goal(path.end().to_node):    # solution found
                self.display(1, self.num_expanded, "paths have been expanded and",
                            len(self.frontier), "paths remain in the frontier")
                self.solution = path   # store the solution found
                return path
            else:
                neighs = self.problem.neighbors(path.end().to_node)
...
class AStarSearcher(Searcher):
    ...
    def add_to_frontier(self,path):
        ...
        value = path.cost+self.problem.heuristic(path.end().to_node)
        self.frontier.add(path, value)

Searcher.max_display_level = 0  # display search trace (0 turns off)
FrontierPQ.max_display_level = 0  # display search trace (0 turns off)
AStarSearcher.max_display_level = 0  # display search trace (0 turns off)
